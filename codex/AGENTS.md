...
2. Create a fresh git worktree for every new task under the project root.
   Use `<project>/worktrees/<task-or-branch-name>` (for example, `project/worktrees/feat-some-feature`).
...
9. Review lessons and memory files before starting and update them when you learn something reusable.
   Project-local lessons and memory must live at the project root, such as `<project>/LESSONS.md` or `<project>/MEMORY.md`.
   Check global `~/.codex/LESSONS.md` for cross-project lessons and project-root lesson/memory files when present.
